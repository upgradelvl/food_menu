...
class Food_Upload extends StatefulWidget {
  const Food_Upload({super.key});

  @override
  State<Food_Upload> createState() => _Food_UploadState();
}

class _Food_UploadState extends State<Food_Upload> {
  var isLoading = false;
  ...
  _apiUploadImage(String name, String content,String caption1) {
    ...
    StorageService.uploadImage(_image!).then((img_url) => {
      _apiCreateFood(name, content, img_url,caption1),
    });
  }

  _apiCreateFood(String name, String content, String img_url,String caption1) {
    var post = Post(name: name, caption: content, img_url: img_url,caption1: caption1);
    RTDBService.addFoood(post).then((value) => {
      _resAddPost(),
    });
  }
